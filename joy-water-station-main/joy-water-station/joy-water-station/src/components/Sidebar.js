...
const Sidebar = ({ sidebarOpen, setSidebarOpen, currentView, setCurrentView, currentUser, handleLogout, products = [], customers = [] }) => {
  ...
  // FIXED: Calculate notifications using useMemo instead of useEffect + setState
  const notifications = useMemo(() => {
    ...
    return lowStockItems + pendingDeliveries;
  }, [products]);
  ...
  const getColorClasses = useCallback((color, isActive = false) => {
    const colors = {
      blue: {
