...
-- Add isHidden column to users table if it doesn't exist
ALTER TABLE users ADD COLUMN isHidden BOOLEAN DEFAULT FALSE;

-- Add loginCount and totalLoginAttempts columns to users table if they don't exist
ALTER TABLE users ADD COLUMN loginCount INT DEFAULT 0;
ALTER TABLE users ADD COLUMN totalLoginAttempts INT DEFAULT 0;
...
-- Run this ALTER statement to add missing columns to existing sales table
ALTER TABLE sales 
ADD COLUMN IF NOT EXISTS transactionType VARCHAR(50) DEFAULT 'Walk-in' AFTER customerType,
ADD COLUMN IF NOT EXISTS deliveryType VARCHAR(50) DEFAULT 'Walk-in' AFTER transactionType;
...
CREATE TABLE IF NOT EXISTS sales (
  ...
  customerId INT,
  customerType ENUM('Regular', 'Wholesale', 'Corporate') DEFAULT 'Regular',
  transactionType VARCHAR(50) DEFAULT 'Walk-in',
  deliveryType VARCHAR(50) DEFAULT 'Walk-in',
  items JSON NOT NULL,
