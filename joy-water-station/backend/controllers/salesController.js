...
exports.createSale = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // SECOND: Insert the sale with formatted date
...
exports.updateSale = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const [originalSale] = await connection.query('SELECT * FROM sales WHERE id = ?', [id]);
      if (originalSale.length === 0) {
        throw new Error('Sale not found');
      }

      const oldSale = originalSale[0];
      const oldItems = typeof oldSale.items === 'string' ? JSON.parse(oldSale.items) : oldSale.items;
      const oldStatus = oldSale.status;

      // If sale was completed, restore old stock first
      if (oldStatus === 'Completed') {
        for (const item of oldItems) {
          if (item.itemId && item.quantity) {
            ...
            await connection.query(
              'INSERT INTO stock_history (itemId, type, quantity, reason, userId) VALUES (?, ?, ?, ?, ?)',
              [item.itemId, 'Stock In', item.quantity, `Sale Updated: ${invoiceId}`, null]
            );
          }
        }
      }
      ...
      // If new status is completed, deduct new stock
      if (status === 'Completed' && Array.isArray(items)) {
        for (const item of items) {
          if (item.itemId && item.quantity) {
            const [itemRows] = await connection.query(
            ...
            if (itemRows.length > 0) {
              ...
              await connection.query(
                'INSERT INTO stock_history (itemId, type, quantity, reason, userId) VALUES (?, ?, ?, ?, ?)',
                [item.itemId, 'Stock Out', item.quantity, `Sale Updated: ${invoiceId}`, null]
              );
            }
          }
        }
