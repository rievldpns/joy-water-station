...
if (currentView === 'sales') {
  console.log('Rendering SalesManagement view');
  return (
    <Layout title="Sales Management">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div style={{ border: '2px solid red', padding: '10px', marginBottom: '20px' }}>
          <h2 style={{ color: 'red', fontWeight: 'bold' }}>DEBUG: SalesManagement Component</h2>
          <p>Current View: {currentView}</p>
          <p>Sales: {Array.isArray(sales) ? sales.length : 'not array'}</p>
          <p>Customers: {Array.isArray(customers) ? customers.length : 'not array'}</p>
          <p>Items: {Array.isArray(items) ? items.length : 'not array'}</p>
          <p>Products: {Array.isArray(products) ? products.length : 'not array'}</p>
        </div>
        <SalesManagement
          sales={sales}
          setSales={setSales}
