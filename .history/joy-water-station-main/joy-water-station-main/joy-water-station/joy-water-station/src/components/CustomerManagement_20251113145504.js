...
// API helper function - token passed as parameter instead of localStorage
...
export default function CustomerManagement() {
  // Token management - you can pass this as a prop or manage it differently
  ...
  useEffect(() => {
    const loadCustomers = async () => {
      setLoading(true);
      try {
        const data = await apiRequest('/customers', authToken);
        setCustomers(data);
        pushToast('success', `Loaded ${data.length} customers from database`);
      } catch (err) {
        console.error('Failed to load customers:', err);
        pushToast('error', `Failed to load customers: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredCustomers = customers.filter(customer => {
    ...
    const matchesStatus = filterStatus === 'All' ||
      (filterStatus === 'Hidden' ? customer.hidden :
       filterStatus === 'Active' ? !customer.hidden :
       false);
  ...
  const activeCustomers = customers.filter(c => !c.hidden).length;
  ...
  const saveEdit = async () => {
    if (!formData.name || !formData.phone || !formData.address) {
      pushToast('error', 'Please fill in required fields: Name, Phone, and Address');
      return;
    }

    try {
      if (newCustomer) {
        const response = await apiRequest('/customers', authToken, {
          method: 'POST',
        ...
        const response = await apiRequest(`/customers/${editingCustomerId}`, authToken, {
          method: 'PUT',
          body: JSON.stringify(formData)
        });
        setCustomers(customers.map(customer =>
          customer.id === editingCustomerId ? response : customer
        ));
      ...
      pushToast('error', err.message || 'Failed to save customer');
    }
  };
  ...
  const confirmHideCustomer = async () => {
    if (customerToArchive) {
      try {
        await apiRequest(`/customers/${customerToArchive}`, authToken, {
          method: 'DELETE'
        });
        // Update local state to mark as hidden
        setCustomers(customers.map(c => 
          c.id === customerToArchive ? { ...c, hidden: true } : c
        ));
        pushToast('success', 'Customer archived successfully');
      } catch (err) {
        console.error('Failed to archive customer:', err);
        pushToast('error', err.message || 'Failed to archive customer');
      }
    }
  ...
  const restoreCustomer = async (id) => {
    try {
      await apiRequest(`/customers/${id}/restore`, authToken, {
        method: 'PATCH'
      });
      // Update local state to mark as not hidden
      setCustomers(customers.map(c => 
        c.id === id ? { ...c, hidden: false } : c
      ));
      pushToast('success', 'Customer restored successfully');
    } catch (err) {
      console.error('Failed to restore customer:', err);
      pushToast('error', err.message || 'Failed to restore customer');
    }
  };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-8">
        ...
        <div className="mt-2 text-sm text-gray-500">
          Connected to database • {customers.length} total customers loaded
        </div>
      </div>
              ...
              {sortedCustomers.length === 0 ? (
                <tr>
                  <td colSpan="9" className="px-6 py-12 text-center text-gray-500">
                    {customers.length === 0 ? 'No customers in database. Click "Add Customer" to create one.' : 'No customers match your filters.'}
                  </td>
                </tr>
              ) : (
                sortedCustomers.map(customer => (
                  <tr key={customer.id} className={customer.hidden ? 'bg-gray-50' : ''}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {editingCustomerId === customer.id ? (
                        ...
                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full
                          ${customer.customerType === 'Regular' ? 'bg-green-100 text-green-800' :
                            customer.customerType === 'Wholesale' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-500">{customer.totalOrders || 0}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                        ${customer.hidden ? 'bg-gray-100 text-gray-800' : 'bg-green-100 text-green-800'}`}>
                        {customer.hidden ? 'Hidden' : 'Active'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      {editingCustomerId === customer.id ? (
