    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (!isUnmountedRef.current) {
          ...
          // Enhanced debug info
          const loggedInCount = data.filter(u => u.loginCount > 0).length;
          const neverLoggedInCount = data.filter(u => !u.loginCount || u.loginCount === 0).length;
          
          setDebugInfo(
            `✓ Successfully loaded ${data.length} users\n` +
            `  • ${loggedInCount} users have logged in\n` +
            `  • ${neverLoggedInCount} users have never logged in\n` +
            `  • ${data.filter(u => u.role === 'Administrator').length} administrators\n` +
            `  • ${data.filter(u => u.isBlocked).length} blocked users`
          );
        }
      }
    } catch (error) {
