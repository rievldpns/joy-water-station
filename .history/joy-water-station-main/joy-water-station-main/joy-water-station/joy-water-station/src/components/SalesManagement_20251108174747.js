...
export default function SalesManagement({ 
  ...
  const todaySales = (sales || []).filter(s => s.date && s.date.startsWith(todayStr)).reduce((sum, s) => sum + (s.total || 0), 0);

  const weekStart = new Date();
  weekStart.setDate(weekStart.getDate() - 7);
  const weekSales = (sales || []).filter(s => s.date && new Date(s.date) >= weekStart).reduce((sum, s) => sum + (s.total || 0), 0);

  const monthStart = new Date();
  monthStart.setDate(1);
  const monthSales = (sales || []).filter(s => s.date && new Date(s.date) >= monthStart).reduce((sum, s) => sum + (s.total || 0), 0);
  ...
  const saveSale = async () => {
    ...
    try {
      ...
      if (editingSaleId) {
        response = await apiRequest(`/sales/${editingSaleId}`, {
          method: 'PUT',
      ...
      pushToast('error', err.message || 'Failed to save sale');
      return;
    }
  };
            ...
            <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <select value={formData.customerId} onChange={(e) => {
                    ...
                    setFormData({
                      ...
                      customerType: selectedCustomer ? selectedCustomer.customerType : 'Regular'
                    });
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={formData.deliveryType} onChange={(e) => setFormData({ ...formData, deliveryType: e.target.value, transactionType: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg">
