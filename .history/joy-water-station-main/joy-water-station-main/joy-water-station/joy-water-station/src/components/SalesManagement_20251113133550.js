...
export default function SalesManagement({ 
  ...
  useEffect(() => {
    ...
    const loadItems = async () => {
      ...
      if (hasAttemptedLoad) return; // Prevent multiple attempts
      setLoading(true);
  ...
  }, [products, setProducts, hasAttemptedLoad]);

  // Listen for item updates dispatched by ItemManagement and update products
  useEffect(() => {
    const handler = (e) => {
    ...
    return () => window.removeEventListener('items:updated', handler);
  }, [setProducts]);

  // Refresh inventory data when modal opens
  useEffect(() => {
    if (showAddModal) {
  ...
  const openAddModal = () => {
    ...
    setFormData({ customerId: '', customerType: 'Regular', transactionType: 'Walk-in', deliveryType: 'Walk-in', items: [{ itemId: '', quantity: 1, price: 0 }], discount: 0, paymentMethod: 'Cash', status: 'Completed', notes: '' });
  ...
  const saveSale = async () => {
    ...
    if (!formData.deliveryType) {
      pushToast('error', 'Please select delivery type');
      return;
    }
    ...
    // Create delivery if delivery type is "Delivery"
    if (formData.deliveryType === 'Delivery') {
      ...
      const deliveryData = {
        ...
        destination: { lat: 7.1907, lng: 125.4553 },
        lastUpdate: new Date(),
        saleId: saleData.id
      };
    ...
    if (editingSaleId) {
      ...
      try {
        const response = await apiRequest(`/sales/${editingSaleId}`, {
          method: 'PUT',
          body: JSON.stringify(saleData)
        });

        setSales(prev => prev.map(sale => sale.id === editingSaleId ? response : sale));

        // Refresh products from API
        const updatedItems = await apiRequest('/items');
        setProducts(Array.isArray(updatedItems) ? updatedItems : []);

        // Notify other components of inventory update
        window.dispatchEvent(new CustomEvent('items:updated', { detail: updatedItems }));

        pushToast('success', 'Sale updated successfully');
      } catch (err) {
        console.error('Failed to update sale:', err);
        pushToast('error', err.message || 'Failed to update sale');
        return;
      }
    } else {
      // Create sale via API
      try {
        ...
        pushToast('error', err.message || 'Failed to create sale');
        return;
      }
    }
  ...
  const confirmDeleteSale = async () => {
    if (saleToDelete) {
      try {
        await apiRequest(`/sales/${saleToDelete}`, {
          method: 'DELETE'
        });
        ...
        pushToast('error', err.message || 'Failed to delete sale');
        return;
      }
    }
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 sticky top-0 bg-white z-10 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={saveSale} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2 font-medium" disabled={loading || products.length === 0}>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={formData.deliveryType} onChange={(e) => setFormData({ ...formData, deliveryType: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg">
                    ...
                    return (
                      <div key={idx} className="grid grid-cols-[2fr_1fr_1fr_40px] gap-2 items-end">
                        <div>
                          ...
                          <select value={it.itemId} onChange={(e) => updateItemRow(idx, 'itemId', e.target.value)} className="w-full px-3 py-2 border rounded bg-white" disabled={loading || !Array.isArray(products)}>
                            <option value="">{loading ? 'Loading items...' : products.length === 0 ? 'No items available' : 'Select Item'}</option>
