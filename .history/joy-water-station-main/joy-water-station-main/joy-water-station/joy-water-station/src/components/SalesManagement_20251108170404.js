...
export default function SalesManagement({ 
  ...
  useEffect(() => {
    ...
    const loadSales = async () => {
      ...
      try {
        ...
        pushToast('error', 'Failed to load sales');
      } finally {
  ...
  useEffect(() => {
    ...
    const loadItems = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        // setProducts if provided by parent
  ...
  const saveSale = async () => {
    ...
    try {
      const bulkUpdates = formData.items.map(it => ({
      ...
      pushToast('error', 'Failed to update inventory stock');
      return;
    }
  ...
  const confirmDeleteSale = async () => {
    if (saleToDelete) {
      try {
        // Delete sale via API (which handles inventory restoration internally)
