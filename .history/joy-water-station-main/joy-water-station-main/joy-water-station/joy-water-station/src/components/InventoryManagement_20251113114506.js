...
const InventoryManagement = () => {
  ...
  const fetchStockHistory = (itemId) => {
    fetch(`http://localhost:5000/api/items/${itemId}/history`)
      .then(response => response.json())
      .then(data => {
  ...
  const getStockStatus = (current, min, max) => {
    if (current === 0) return { status: 'Out of Stock', color: 'red', bgClass: 'bg-red-500', textClass: 'text-red-700', bgLightClass: 'bg-red-100' };
    if (current <= min) return { status: 'Low Stock', color: 'yellow', bgClass: 'bg-yellow-500', textClass: 'text-yellow-700', bgLightClass: 'bg-yellow-100' };
    return { status: 'Sufficient', color: 'green', bgClass: 'bg-green-500', textClass: 'text-green-700', bgLightClass: 'bg-green-100' };
  };
  ...
  const saveEdit = () => {
    ...
    fetch(`http://localhost:5000/api/items/${editingItemId}`, {
      method: 'PUT',
      headers: {
    ...
    .then(updatedItem => {
      const updatedProducts = products.map(product =>
        product.id === editingItemId ? updatedItem : product
      );
  ...
  const saveStockEntry = () => {
    ...
    fetch(`http://localhost:5000/api/items/${selectedItem.id}/stock-entry`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
                ...
                <tr key={item.id} className={editingItemId === item.id ? 'bg-yellow-50' : ''}>
                  {editingItemId === item.id ? (
                    <>
                      ...
                      <td className="px-6 py-2">
                        {(() => {
                          const status = getStockStatus(parseInt(formData.currentStock) || 0, item.minStock, item.maxStock);
                        return (
