...
import { Edit2, Save, X, UserX, UserCheck, Trash2, Search, ChevronDown, ChevronUp, Key } from 'lucide-react';
...
export default function UserManagement({ users, setUsers, currentUser, setCurrentView }) {
  ...
  const startChangePassword = (user) => {
    ...
    if (user.role === 'Administrator') {
      alert('Cannot change password for administrator accounts');
      return;
    }
  ...
  const saveChangePassword = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/users/${changingPasswordUserId}/change-password`, {
        method: 'PUT',
        headers: {
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200">
            {sortedUsers.map(user => (
              <tr key={user.id} className={editingUserId === user.id || changingPasswordUserId === user.id ? 'bg-yellow-50' : 'hover:bg-gray-50'}>
                {editingUserId === user.id ? (
                  ...
                  <>
                    <td className="px-6 py-2" colSpan={currentUser?.role === 'Administrator' ? '9' : '8'}>
            ...
            {sortedUsers.length === 0 && (
              <tr>
                <td colSpan={currentUser?.role === 'Administrator' ? '9' : '8'} className="text-center py-8 text-gray-500">
