...
import { Edit2, Save, X, UserX, UserCheck, Trash2, Search, ChevronDown, ChevronUp, Key, RefreshCw, Clock, AlertCircle, CheckCircle } from 'lucide-react';
...
function UserManagement({ currentUser, setCurrentView }) {
  ...
  const fetchLoginHistory = useCallback(async () => {
    ...
    try {
      ...
      console.error('Network error fetching login history:', error);
      if (!isUnmountedRef.current) {
        setError('Network error. Please check your connection.');
      }
    } finally {
  ...
  // Fetch login history when switching to history tab
  useEffect(() => {
  ...
  const filteredLoginHistory = React.useMemo(() => {
  ...
  }, [loginHistory, searchTerm, filterStatus]);
  ...
  const sortedLoginHistory = React.useMemo(() => {
    return [...filteredLoginHistory].sort((a, b) => {
      if (!sortField) return 0;
      let aVal = a[sortField];
      let bVal = b[sortField];
      if (aVal === null || aVal === undefined) aVal = '';
  ...
  }, [filteredLoginHistory, sortField, sortDirection]);
  ...
  const loginStats = React.useMemo(() => ({
    ...
    successfulLogins: loginHistory.filter(l => l.success).length,
    failedLogins: loginHistory.filter(l => !l.success).length,
    uniqueUsers: new Set(loginHistory.map(l => l.userId).filter(Boolean)).size
  }), [loginHistory]);
  ...
  }, [fetchUsers, fetchLoginHistory, activeTab]);

  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold">Logged-in Users</h1>
        <button
                      ...
                      <tr key={user.id} className={`${editingUserId === user.id || changingPasswordUserId === user.id ? 'bg-yellow-50' : 'hover:bg-gray-50'} transition-colors`}>
                        {editingUserId === user.id ? (
                          <>
                            ...
                            <td className="px-3 py-2 text-sm">{formData.lastLogin || 'N/A'}</td>
                            <td className="px-3 py-2">
                          ...
                          <>
                            <td className="px-3 py-2 text-sm font-medium">{user.username}</td>
                            ...
                            <td className="px-3 py-2 text-sm font-medium text-green-600">{loginCount}</td>
                            <td className="px-3 py-2 text-sm font-medium">{totalAttempts}</td>
                            <td className="px-3 py-2 text-sm">{user.lastLogin || 'N/A'}</td>
                            <td className="px-3 py-2">
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-white p-4 rounded shadow">
              ...
              <p className="text-xl font-bold">{loginStats.totalAttempts}</p>
            </div>
            <div className="bg-white p-4 rounded shadow">
              <p className="text-sm text-gray-600">Successful Logins</p>
              <p className="text-xl font-bold text-green-600">{loginStats.successfulLogins}</p>
            </div>
            <div className="bg-white p-4 rounded shadow">
              <p className="text-sm text-gray-600">Failed Attempts</p>
              <p className="text-xl font-bold text-red-600">{loginStats.failedLogins}</p>
            </div>
            <div className="bg-white p-4 rounded shadow">
              <p className="text-sm text-gray-600">Unique Users</p>
              <p className="text-xl font-bold">{loginStats.uniqueUsers}</p>
            </div>
          </div>
          ...
          <div className="overflow-x-auto bg-white rounded shadow">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  <th
                    ...
                    className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer select-none hover:bg-gray-200 transition-colors"
                  >
                    <div className="flex items-center gap-1">
                      Login Time
                      {sortField === 'loginTime' && (
              ...
              <tbody className="divide-y divide-gray-200">
                {sortedLoginHistory.length === 0 && !loading ? (
                  <tr>
                    <td colSpan="7" className="text-center py-8 text-gray-500">
                      {loginHistory.length === 0 ? 'No login history found.' : 'No login history matching your criteria.'}
                    </td>
                  </tr>
                ) : (
                  sortedLoginHistory.map(log => (
                    <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-3 py-2 text-sm whitespace-nowrap">{log.loginTime}</td>
                      <td className="px-3 py-2 text-sm font-medium">{log.username || 'Unknown'}</td>
                      <td className="px-3 py-2 text-sm">{log.email || 'N/A'}</td>
                      <td className="px-3 py-2">
                        {log.role && (
                          <span className={`px-2 py-1 rounded text-xs font-medium ${log.role === 'Administrator' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'}`}>
                            {log.role}
                          </span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-sm font-mono">{log.ipAddress}</td>
                      <td className="px-3 py-2">
                        {log.success ? (
                      ...
                      <td className="px-3 py-2 text-sm max-w-xs truncate" title={log.userAgent}>
                        {log.userAgent}
                      </td>
                    </tr>
                  ))
