...
// API helper function without authentication
...
export default function CustomerManagement() {
  ...
  const filteredCustomers = customers.filter(customer => {
    ...
    const matchesStatus = filterStatus === 'All' ||
      (filterStatus === 'Hidden' ? customer.hidden :
       filterStatus === 'Active' ? !customer.hidden : false);
  ...
  const saveEdit = async () => {
    ...
    try {
      if (newCustomer) {
        ...
        const response = await apiRequest(`/customers/${editingCustomerId}`, {
          method: 'PUT',
          body: JSON.stringify(formData)
        });
  ...
  const confirmHideCustomer = async () => {
    if (customerToArchive) {
      try {
        await apiRequest(`/customers/${customerToArchive}`, {
          method: 'DELETE'
        });
  ...
  const restoreCustomer = async (id) => {
    try {
      await apiRequest(`/customers/${id}/restore`, {
        method: 'PATCH'
      });
      setCustomers(customers.map(c => 
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">http://localhost:5000/api/customers</p>
        </div>
      </div>
    );
  }
