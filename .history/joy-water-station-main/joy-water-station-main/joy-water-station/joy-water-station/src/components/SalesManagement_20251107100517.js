...
export default function SalesManagement({ 
  ...
  useEffect(() => {
    ...
    const loadItems = async () => {
      ...
      try {
        ...
        console.log('Products set, setting loading to false');
      } catch (err) {
    ...
    const handler = (e) => {
      console.log('Items updated event received:', e.detail);
  ...
  // Debug: Log when component mounts and key state changes
  useEffect(() => {
    console.log('SalesManagement mounted/updated, loading:', loading, 'products:', products?.length);
  }, [loading, products]);
  ...
  const openAddModal = () => {
    console.log('openAddModal called, loading:', loading);
    if (loading) {
      console.log('Cannot open modal while loading');
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-yellow-100 p-4 rounded-lg border border-yellow-300 mb-4">
        ...
        <p>Products loaded: {Array.isArray(products) ? products.length : 'not array'}</p>
        <p>Modal states: showAddModal={showAddModal}, showDeleteModal={showDeleteModal}, viewSale={!!viewSale}, showAlertModal={showAlertModal}</p>
        <p>Toasts count: {toasts.length}</p>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center gap-2 font-medium shadow"
            disabled={loading}
          >
            <Plus className="w-4 h-4" /> New Sale {loading && '(Loading...)'}
          </button>
          <button
            onClick={() => {
            ...
            className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 flex items-center gap-2 font-medium shadow"
          >
            Test Click
          </button>
          <button
            ...
            className="bg-white border border-gray-300 px-3 py-2 rounded-lg hover:bg-gray-50 flex items-center gap-2 font-medium shadow"
          >
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 sticky top-0 bg-white z-10 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={saveSale} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2 font-medium" disabled={loading}>
