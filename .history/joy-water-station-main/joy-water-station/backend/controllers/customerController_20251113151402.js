...
const getAllCustomers = async (req, res) => {
  try {
    ...
    const [customers] = await db.promise().query('SELECT * FROM customers ORDER BY createdAt DESC');
    res.json(customers);
  } catch (error) {
...
const getCustomerById = async (req, res) => {
  try {
    const { id } = req.params;
    const [customers] = await db.promise().query('SELECT * FROM customers WHERE id = ?', [id]);
...
const createCustomer = async (req, res) => {
  try {
    ...
    const [newCustomer] = await db.promise().query('SELECT * FROM customers WHERE id = ?', [result.insertId]);
...
const updateCustomer = async (req, res) => {
  try {
    ...
    const [updatedCustomer] = await db.promise().query('SELECT * FROM customers WHERE id = ?', [id]);
...
const deleteCustomer = async (req, res) => {
  try {
    const { id } = req.params;
    await db.promise().query('UPDATE customers SET hidden = TRUE WHERE id = ?', [id]);
...
const restoreCustomer = async (req, res) => {
  try {
    const { id } = req.params;
    await db.promise().query('UPDATE customers SET hidden = FALSE WHERE id = ?', [id]);
