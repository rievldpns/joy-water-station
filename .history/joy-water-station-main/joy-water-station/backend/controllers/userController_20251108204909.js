...
// Get all users (admin only) - Shows all users including admins
exports.getAllUsers = async (req, res) => {
  try {
    // Get all users that are not hidden
    const [users] = await db.promise().query(
      'SELECT id, username, email, firstName, lastName, phone, address, role, createdAt, lastLogin, isBlocked FROM users WHERE isHidden = FALSE ORDER BY createdAt DESC'
    );
