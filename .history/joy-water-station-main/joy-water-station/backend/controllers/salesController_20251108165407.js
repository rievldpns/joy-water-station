    ...
    try {
      ...
      const [result] = await connection.query(
        `INSERT INTO sales
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [invoiceId, date, customerId, customerType, transactionType, deliveryType, JSON.stringify(items), subtotal, discount, total, paymentMethod, status, notes]
      );
      ...
      if (status === 'Completed' && Array.isArray(items)) {
        for (const item of items) {
          if (item.itemId && item.quantity) {
            // Check if we should update items or products table
            // Try items first
            ...
            if (itemRows.length > 0) {
              // Update items table
              const newStock = itemRows[0].currentStock - item.quantity;
              if (newStock < 0) {
              ...
              if (productRows.length > 0) {
                const newStock = productRows[0].currentStock - item.quantity;
                if (newStock < 0) {
  ...
  try {
    await db.promise().query(
      `UPDATE sales SET
       ...
       WHERE id = ?`,
      [invoiceId, date, customerId, customerType, transactionType, deliveryType, JSON.stringify(items), subtotal, discount, total, paymentMethod, status, notes, id]
    );
