      ...
      if (status === 'Completed' && Array.isArray(items)) {
        for (const item of items) {
          if (item.itemId && item.quantity) {
            ...
            // Check if we should update items table
            ...
            if (itemRows.length > 0) {
              ...
              // Try products table if item not found
              const [productRows] = await connection.query(
                'SELECT currentStock, name FROM products WHERE id = ?',
                [item.itemId]
              );
              
              if (productRows.length > 0) {
                const currentStock = productRows[0].currentStock;
                const productName = productRows[0].name;
                const newStock = currentStock - item.quantity;
                
                if (newStock < 0) {
                  throw new Error(`Insufficient stock for product: ${productName}. Available: ${currentStock}, Requested: ${item.quantity}`);
                }
                ...
                console.warn(`Item/Product with ID ${item.itemId} not found`);
              }
            }
          }
        }
