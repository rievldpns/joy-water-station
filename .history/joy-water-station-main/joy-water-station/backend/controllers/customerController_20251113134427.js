...
const getAllCustomers = async (req, res) => {
  try {
    const [customers] = await db.query('SELECT * FROM customers WHERE hidden = FALSE ORDER BY createdAt DESC');
    res.json(customers);
  } catch (error) {
...
const getCustomerById = async (req, res) => {
  try {
    const { id } = req.params;
    const [customers] = await db.query('SELECT * FROM customers WHERE id = ? AND hidden = FALSE', [id]);
...
const createCustomer = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO customers (name, phone, address, email, customerType) VALUES (?, ?, ?, ?, ?)',
...
const updateCustomer = async (req, res) => {
  try {
    ...
    await db.query(
      'UPDATE customers SET name = ?, phone = ?, address = ?, email = ?, customerType = ? WHERE id = ? AND hidden = FALSE',
      [name, phone, address, email || '', customerType || 'Regular', id]
    );

    const [updatedCustomer] = await db.query('SELECT * FROM customers WHERE id = ? AND hidden = FALSE', [id]);
