...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (users.length === 0) {
      // Log failed login attempt for non-existent user (use userId 0 or NULL)
      const ipAddress = req.ip || req.connection.remoteAddress || req.socket.remoteAddress || 'unknown';
      const userAgent = req.get('User-Agent') || 'unknown';

      await db.promise().query(
        'INSERT INTO login_logs (userId, ipAddress, userAgent, success) VALUES (NULL, ?, ?, FALSE)',
        [ipAddress, userAgent]
      );
    ...
    // Fetch updated user data with login counts (both successful and total)
    const [updatedUsers] = await db.promise().query(
      `SELECT
        ...
        u.isBlocked,
        COUNT(CASE WHEN ll.success = TRUE THEN 1 END) as loginCount,
        COUNT(ll.id) as totalLoginAttempts,
        COUNT(CASE WHEN ll.success = FALSE THEN 1 END) as failedLoginAttempts
      FROM users u
      LEFT JOIN login_logs ll ON u.id = ll.userId
      WHERE u.id = ?
      GROUP BY u.id`,
      [user.id]
    );
...
// Get all users - Shows all registered users with complete login statistics
...
// NEW: Get complete login history for all users
  ...
  try {
    const [loginHistory] = await db.promise().query(
      `SELECT
        ...
        ll.userAgent,
        ll.success,
        DATE_FORMAT(ll.loginTime, "%Y-%m-%d %H:%i:%s") as loginTime
      FROM login_logs ll
...
// NEW: Get login history for a specific user
