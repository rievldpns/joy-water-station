...
// Get all users - Shows all users who have logged in (past and present)
  ...
  try {
    const [users] = await db.promise().query(
      ...
      WHERE (u.isHidden = FALSE OR u.isHidden IS NULL)
        AND (u.lastLogin IS NOT NULL OR u.id IN (
          SELECT DISTINCT userId FROM login_logs WHERE userId IS NOT NULL AND success = TRUE
        ))
      GROUP BY u.id
